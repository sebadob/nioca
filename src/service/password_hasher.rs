...
static ARGON2_PARAMS: Lazy<argon2::Params> = Lazy::new(|| {
    let mut argon2_m_cost = env::var("ARGON2_M_COST")
        .unwrap_or_else(|_| String::from("262144"))
        .parse::<u32>()
...
impl HashPassword {
    pub async fn hash_password(plain_text: &str, pepper: &str) -> Result<String, ErrorResponse> {
        let (tx, rx) = flume::unbounded();
        let plain = format!("{}{}", plain_text, pepper);
        let s = Self {
            plain_text: plain,
            tx,
            created: Instant::now(),
...
pub async fn run() {
    debug!("Password Hash Limiter started");
    while let Ok(msg) = HASH_CHANNELS.1.recv_async().await {
        let res = match msg {
            PasswordHashMessage::Hash(m) => {
                check_await_threshold(&m.created);
                tokio::task::spawn_blocking(move || hash_password(m)).await
            }
            PasswordHashMessage::Compare(m) => {
                check_await_threshold(&m.created);
                tokio::task::spawn_blocking(move || compare_passwords(m)).await
            }
        };
        if let Err(err) = res {
            error!("{}", err);
...
mod tests {
    ...
    async fn test_limiter() {
        ...
        let pepper = "SomePepper";

        let start = Instant::now();
        HashPassword::hash_password(plain, pepper).await.unwrap();
        ...
        let start = Instant::now();
        let handle_1 =
            tokio::spawn(async { HashPassword::hash_password(plain, pepper).await.unwrap() });
        let handle_2 =
            tokio::spawn(async { HashPassword::hash_password(plain, pepper).await.unwrap() });
        let handle_3 =
            tokio::spawn(async { HashPassword::hash_password(plain, pepper).await.unwrap() });
    ...
    #[tokio::test]
    async fn test_hasher_with_pepper() {
        tokio::spawn(async move { run().await });

        let input = "SuperSafe123";
        let pepper = "SomePepper";
        let hash = HashPassword::hash_password(input, pepper).await.unwrap();
        let res = ComparePasswords::is_match(input, hash, pepper)
            .await
            .unwrap();
        assert_eq!(res, true);
    }
